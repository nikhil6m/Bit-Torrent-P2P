...
public class Peer2 {
	...
	void execute(int fileOwnerPortNumber , int peerPortNumber , int neighborPortNumber) throws Exception {
		try {
			...
			while (chunksAllotted > 0) {
				...
				File file = new File("C:\\Users\\NIKHIL MALLADI\\Desktop\\CN1\\Peer2", Integer.toString(chunkName));
			...
			Thread thread = new Thread(new Runnable() {
				public void run() {
					try {
						...
						new Handler2(receiveSocket.accept()).start();
			...
			while (true) {
				System.out.println("Peer2 receiving chunks");
	...
	public static void mergeFiles() throws IOException {
		String directoryPath = "C:\\Users\\NIKHIL MALLADI\\Desktop\\CN1\\Peer2";
...
class Handler2 extends Thread {
	...
	public void run() {
		try {
			System.out.println("Peer2 sending file chunks");
			Thread.sleep(3000);
