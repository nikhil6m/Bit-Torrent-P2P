...
public class Peer4 {
	...
	void execute(int fileOwnerPortNumber , int peerPortNumber , int neighborPortNumber) throws Exception {
		try {
			...
			while (chunksAllotted > 0) {
				...
				File file = new File("C:\\Users\\NIKHIL MALLADI\\Desktop\\CN1\\Peer4", Integer.toString(chunkName));
			...
			Thread thread = new Thread(new Runnable() {
				public void run() {
					try {
						...
						new Handler4(receiveSocket.accept()).start();
			...
			while (true) {
				System.out.println("Peer4 receiving chunks");
				if (!chunksList.isEmpty()) {
					...
					for (int i = 0; i < a.length; i++) {
						...
						if (status) {
							File file = new File("C:\\Users\\NIKHIL MALLADI\\Desktop\\CN1\\Peer4",
	...
	public static void mergeFiles() throws IOException {
		String directoryPath = "C:\\Users\\NIKHIL MALLADI\\Desktop\\CN1\\Peer4";		
...
class Handler4 extends Thread {
	...
	public Handler4(Socket connection) throws IOException {
		this.connection = connection;
		this.chunkLoc = "C:\\Users\\NIKHIL MALLADI\\Desktop\\CN1\\Peer4";
	}

	public void run() {
		try {
			System.out.println("Peer4 sending file chunks");
			Thread.sleep(3000);
